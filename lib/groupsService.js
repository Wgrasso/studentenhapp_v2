...
/**
 * Sets a group as the user's favorite
 * @param {string} groupId - ID of the group to set as favorite
 * @returns {Promise<Object>} - Success/error response
 */
export const setFavoriteGroup = async (groupId) => {
  try {
    console.log('⭐ Setting favorite group:', groupId);
    
    const { data: { user }, error: userError } = await supabase.auth.getUser();
    if (userError || !user) {
      return {
        success: false,
        error: 'You must be signed in to set a favorite group'
      };
    }

    // Update user metadata to store favorite group
    const { data, error } = await supabase.auth.updateUser({
      data: { favorite_group: groupId }
    });

    if (error) throw error;

    console.log('✅ Successfully set favorite group');
    return {
      success: true,
      message: 'Successfully set as favorite group'
    };
    ...
    return {
      success: false,
      error: error.message || 'Failed to set favorite group. Please try again.'
    };
  }
};

/**
 * Gets the user's favorite group ID from metadata
 * @returns {Promise<string|null>} - Favorite group ID or null
 */
export const getFavoriteGroupId = async () => {
  try {
    ...
    console.error('❌ Error getting favorite group:', error);
    return null;
  }
}; 