...
export const createGroupInSupabase = async (groupName, description = '') => {
  ...
  const createPromise = async () => {
    try {
      ...
      const maxAttempts = 3; // Reduced from 5 to prevent long delays
      let attempt = 1;
      let groupData = null;
...
export const deleteGroup = async (groupId) => {
  ...
  const deletePromise = async () => {
    try {
      ...
      if (group.created_by !== user.id) {
        console.log('❌ User is not the creator of this group');
        return {
          success: false,
      ...
      console.log('✅ User verified as group creator');

      // STEP 1: Clean up meal requests and related data
      console.log('🧹 Cleaning up meal requests, options, and votes...');
      
      // Get all meal requests for this group
      const { data: mealRequests, error: mealRequestsError } = await supabase
        .from('meal_requests')
        .select('id')
        .eq('group_id', groupId);

      if (mealRequestsError) {
        console.warn('⚠️ Could not fetch meal requests for cleanup:', mealRequestsError);
      } else if (mealRequests && mealRequests.length > 0) {
        console.log(`🗑️ Found ${mealRequests.length} meal requests to clean up`);
        
        // Delete votes first (they reference meal_request_options)
        for (const request of mealRequests) {
          ...
          if (votesError) {
            console.warn(`⚠️ Failed to delete votes for request ${request.id}:`, votesError);
          }
        }
        ...
        for (const request of mealRequests) {
          ...
          if (optionsError) {
            console.warn(`⚠️ Failed to delete meal options for request ${request.id}:`, optionsError);
          }
        }
        ...
        if (requestsDeleteError) {
          console.warn('⚠️ Failed to delete meal requests:', requestsDeleteError);
        } else {
          console.log('✅ Meal requests and related data cleaned up');
        }
      }

      // STEP 2: Remove all group members
      console.log('🧹 Cleaning up group members...');
      ...
      if (membersError) {
        console.warn('⚠️ Failed to clean up group members:', membersError);
      } else {
        console.log('✅ Group members cleaned up');
      }

      // STEP 3: HARD DELETE the group from Supabase
      console.log('🗑️ Permanently deleting group from database...');
      ...
      if (deleteError) {
        ...
        return {
          success: false,
          error: `Failed to delete group: ${deleteError.message}`
        };
      }
      ...
      return {
        success: false,
        error: error.message || 'Failed to delete group. Please try again.'
      };
    }
  };
  ...
  try {
    ...
    console.error('❌ [SERVICE] Delete group failed:', error);
          return {
        success: false,
        error: error.message === 'Delete group service timeout after 8 seconds'
          ? 'Deleting is taking too long. Please check your connection and try again.'
